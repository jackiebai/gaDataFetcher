...
class GaService {
    ...
    def getData(appId) {
        ...
        def query = [ids: "ga:$appId", 'start-date': "365daysAgo", 'end-date': "yesterday", metrics: "ga:users", dimensions: "ga:operatingSystem,ga:operatingSystemVersion", access_token: accessToken]
        def json = makeGaRequest(query)
        if (json.rows?.size() > 0) {//if we have rows and there are more than 0 of them
            for (def row : json.rows) {
                def deviceOsUsers = new DeviceOsUsers(appId: appId, deviceType: row[0], osVersion: row[1], totalUserCount: row[2])
                deviceOsUsers.save()
            }
        }
    }
